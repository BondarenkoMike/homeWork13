...
function myPrint(a, b, res) {
  return `${a}^${b}=${res}`; //.toString(a, b, res);
};
...
let car = {
  ...
  engine: 2000,
  model: 'Lacetti',
  _yearNow: new Date().getFullYear(),
  info: getInfo,
  year: 2010,
  get used() {
    return (this.year < this._yearNow) ? USED_CAR : NEW_CAR;
  },
  set used(value) {
    if (CAR_STATUS.includes(value.toLowerCase())) {
      this._used = value;
      if ((value.toLowerCase() === NEW_CAR) && (this.year !== this._yearNow)) {
        this.year = this._yearNow;
      };
    } else {
...
let car2 = {
  ...
  set used(value) {
    if (CAR_STATUS.includes(value.toLowerCase())) {
      this._used = value;
      if ((value.toLowerCase() === 'new') && (this.year !== this._yearNow)) {
        this.year = this._yearNow;
      };
    } else {
      throw `'${value}' - Invalid Value. Just "${NEW_CAR}" and "${USED_CAR}" allowed.`;
...
var myTriple = myMul.bind(this, 3);
...
function myUniq(arr) {
  let uv = new Set;
  arr.forEach((el, index, arr) => uv.add(el), null);
  return uv;
}
